...
export const projectToolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      'Capture a PNG screenshot of the running Godot viewport. Requires an active runtime session. responseMode defaults full (current behavior: full inline PNG); preview saves the original but returns a 960x540-bounded preview inline; path_only returns metadata only. Screenshots are saved under .mcp/screenshots. Errors if no session is active or the bridge times out.',
    annotations: { readOnlyHint: true },
    inputSchema: {
      type: 'object',
      properties: {
...
export async function handleTakeScreenshot(runner: GodotRunner, args: OperationParams) {
  ...
  if (responseMode === 'preview') {
    commandParams.previewMaxWidth = previewMaxWidth;
    commandParams.previewMaxHeight = previewMaxHeight;
  }
  ...
  try {
    ...
    content.push({ type: 'text', text: `Screenshot saved to: ${parsed.path}` });
