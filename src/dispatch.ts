...
export const toolDispatch: Record<string, ToolHandler> = {
  ...
  detach_project: (runner, args) => handleDetachProject(runner, args),
  get_debug_output: (runner, args) => handleGetDebugOutput(runner, args),
  stop_project: (runner, args) => handleStopProject(runner, args),
  list_runtime_sessions: (runner) => handleListRuntimeSessions(runner),
